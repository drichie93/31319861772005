<div class="workspace">
  <div class="w3-row header">
    <div class="w3-third">
        <div class="hdtitle">
            TASK MANAGER
        </div>
    </div>

    <div class="w3-twothird">
        <nav>
            <a href="index.php" class=" w3-hover-none w3-border-black  w3-hover-border-green">Create Task</a>
            <a href="alltask.php" class=" w3-hover-none w3-border-black  w3-hover-border-green">All Tasks</a>
            <a href="newtask.php" class="w3-hover-none w3-border-black  w3-hover-border-green">New Tasks</a>
            <a href="closedtask.php" class="w3-hover-none w3-border-black  w3-hover-border-green">Closed Tasks</a>
            <a href="newEmployee.php" class="w3-hover-none w3-border-black w3-hover-border-green">Add Employee</a>
        </nav>
    </div>
  <div>
</div>
</div>
</div>
